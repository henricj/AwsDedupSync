...
#pragma unmanaged
    ...
    {
        if (HashReturn::SUCCESS != Keccak_HashInitialize_SHA3_512(&hash))
            return false;
    ...
    bool Validate(Keccak_HashInstance& hash, const TestVector * testVector)
    {
        ...
        if (HashReturn::SUCCESS != Keccak_HashUpdate(&hash, testVector->Message, testVector->BitLength))
        {
        ...
        auto isOk = std::equal(std::begin(buffer), std::end(buffer), stdext::make_unchecked_array_iterator(testVector->Hash));
...
#pragma managed
    ...
    void Keccak::Validate()
    {
        if (!UnmanagedValidate())
            throw gcnew System::Security::Cryptography::CryptographicException(L"Validation failed");
    }
    ...
    void Keccak::Initialize()
    {
        ...
        if (HashReturn::SUCCESS != ret)
            throw gcnew System::Security::Cryptography::CryptographicException(L"Internal error");
    ...
    void Keccak::HashCore(cli::array<unsigned char> ^ buffer, int offset, int length)
    {
    ...
    cli::array<unsigned char> ^ Keccak::HashFinal()
    {
        cli::array<unsigned char>^ buffer = gcnew cli::array<unsigned char>(hash_->fixedOutputLength / 8);
